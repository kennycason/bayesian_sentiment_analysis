...
class StochasticBayesianClassifierImdbSampleTest {
    ...
    @Test
    fun stochasticClassifierCompositeBiGramAndTriGram() {
        logger.info("bigram + trigramgram classifier")
        val classifier = StochasticBayesianClassifier(classifierCount = 15, samplingPercent = 0.5f)
        val imdbDataCorpus = ImdbLoader().loadFromResourceDirectory(resourcePath)
        val tokenizer = CompositeGramTokenizer<String>(Lists.mutable.of(NGramTokenizer<String>(2), NGramTokenizer<String>(3)))
